...
if (!empty($_POST['submit'])) {
    ...
    if ($name != $user['name'] || $email != $user['email']) {
        ...
        echo '<script>alert("Cập nhật thành công!");window.location.href="../profile.php?id='.$user['id'].'"</script>';
    }
    else {
...
if (isset($_POST['doiMatKhau'])) {//if (!empty($_POST['doiMatKhau'])) {
    $id = $_POST['id'];
    $name = $_POST['name'];
    $email = $_POST['mail'];
    $passold = password_verify($_POST['passwordOld'], $user['password']);
    $passnew1 = $_POST['passwordNew1'];//password_hash($_POST['passwordNew1'], PASSWORD_DEFAULT);
    $passnew2 = $_POST['passwordNew2'];//password_hash($_POST['passwordNew2'], PASSWORD_DEFAULT);
    
    if ($passold == true) {
        if ($passnew1 == $passnew2) {
            $pass = password_hash($passnew1, PASSWORD_DEFAULT);
            $userModel->updateUsers($name, $email, $pass, $id);
            echo "<script>alert('Đổi mật khẩu thành công!');window.location.href='../profile.php?id=".$user['id']."'</script>";
        }
        else {
                ...
                <div class="content-profile">
                    <form method="post">
                        ...
                        <div class="form-group row">
                            ...
                            <input class="form-control col-md-3" name="username" placeholder="UserName" value='<?php if (!empty($user['username'])) echo $user['username']; ?>' readonly>
                        </div>
                        <div class="form-group row">
                        ...
                        <div class="form-group row">
                            ...
                            <input type="password" class="form-control col-md-3" name="password" placeholder="Password" value="<?php if (!empty($user['password'])) echo $user['password']; ?>"readonly>
                            <div class="box">
